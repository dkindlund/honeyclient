		...
		if(dwBytesReturned > 0)
		{
			...
			{		
				...
				if(!CheckIfExcluded(processPath)) 
				{
					...
					{
						NotifyListeners(p.bCreate, parentProcessPath, processPath, L"");
					} else {
						//printf("excluded: %ls\n", processPath.c_str());
					}
				}		
...
void
ProcessMonitor::NotifyListeners(BOOLEAN bCreate, wstring processParentPath, wstring processPath, wstring extraData)
{
	...
	{
		(*lit)->OnProcessEvent(bCreate, processParentPath, processPath, extraData);
	}
}
...
void
ProcessMonitor::OnVisitEvent(int eventType, wstring url, wstring visitor)
{
