...
{
	//char szMsg[14096];
	//strcpy_s( szMsg, "Hello world from " );
	...
	{		
		if(!serverConnection->sendQueue->empty())
		{
			...
			//char szMsg[msg.length()];
			//ZeroMemory(&szMsg, sizeof(szMsg));
			int len = wcstombs( szMsg, msg.c_str(), msg.length() );

			//errno_t err = wcstombs_s( &len, szMsg, msg.size() +1 , msg.c_str(), _TRUNCATE );

			if(send(serverConnection->client, szMsg,len, 0) == SOCKET_ERROR)
			{
...
ServerReceive::Run()
{
	...
	while( bytesRecv != SOCKET_ERROR && serverConnection->connected)
	{
		...
		bytesRecv = recv(serverConnection->client, buff, 100, 0);
		if(bytesRecv == 0)
		{
			printf( "Disconnected from server\n");
			serverConnection->connected = false;
