...
FileMonitor::Run()
{
	...
	while(true)
	{
		...
		ZeroMemory(&fEvent, sizeof(FILE_EVENT)*500);

		hResult = FilterSendMessage( commPort,
                                     &command,
                                     sizeof( FILEMONITOR_COMMAND ),
                                     fEvent,
                                     sizeof(FILE_EVENT)*500,
                                     &bytesReturned );
		for(unsigned int i = 0; i < (bytesReturned/sizeof(FILE_EVENT)); i++)
		{
			...
			if(!Monitor::EventIsAllowed(eventName,processPath,filePath))
			{
				...
				NotifyListeners(eventName, processPath, filePath, L"");
			}
		}
		Sleep(500);
	}
...
void
FileMonitor::NotifyListeners(wstring eventType, wstring processFullPath, wstring eventPath, wstring extraData)
{
	...
	{
		(*lit)->OnFileEvent(eventType, processFullPath, eventPath, extraData);
	}
}
