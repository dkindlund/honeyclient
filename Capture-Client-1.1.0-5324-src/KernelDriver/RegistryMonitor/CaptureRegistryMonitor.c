	...
	try
	{
		...
		{		
			...
			if(NT_SUCCESS(create->Status)) {
				...
				RtlStringCbCatW(rEventPacket->event.name, 512, L" - ");
			} else {
			...
			{
				RtlStringCbCatW(rEventPacket->event.name, 512, del->ValueName->Buffer);
			}
			...
			//This is where I split the name from the path, which it didn't do before - X
			RtlStringCbCatW(rEventPacket->event.name, 512, L" - ");
			if(set->ValueName->Length > 0) 
			{
				RtlStringCbCatNW(rEventPacket->event.name, 512,set->ValueName->Buffer,set->ValueName->Length);
			}
			rEventPacket->event.type = RegNtPreSetValueKey;	
			//XENO below (and with the 512s)
			rEventPacket->event.regType = set->Type;
			rEventPacket->event.regDataSize = set->DataSize;
			//copy to userspace the values for the data type
			switch(rEventPacket->event.regType){
				//Strings: REG_SZ, REG_EXPAND_SZ, REG_MULTI_SZ
				case 1:
				case 2:
				case 7:
					RtlStringCbCopyNW(
						(LPWSTR)rEventPacket->event.regData, 
						sizeof(rEventPacket->event.regData)/sizeof(rEventPacket->event.regData[0]),
						set->Data, 
						set->DataSize);
				break;
				default:
					//Everything else, copy the data raw
					if(set->DataSize > 256){
						RtlCopyBytes(rEventPacket->event.regData,set->Data,256);
					}
					else{
						RtlCopyBytes(rEventPacket->event.regData,set->Data,set->DataSize);
					}
				break;
			}
			...
			{
				RtlStringCbCatW(rEventPacket->event.name, 512, L"\\");
				RtlStringCbCatNW(rEventPacket->event.name, 512,enumerate->ValueName->Buffer, enumerate->ValueName->Length);
			}
